...
@section('content')
   ...
   <div style="height: 3cm; background-color: #f8f9fa; padding: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); margin-bottom: 20px; margin-top: 5px;">
         ...
         @foreach($announcements as $announcement)
            <div style="margin-bottom: 15px; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
               ...
               <small>Start Date: {{ \Carbon\Carbon::parse($announcement->startDate)->format('jS F Y') }}
                  @if($announcement->endDate) | End Date: {{ \Carbon\Carbon::parse($announcement->endDate)->format('jS F Y') }} @endif
               </small>
   ...
   <div style="background-color: #f8f9fa; padding: 20px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
       ...
       <div style="display: flex; align-items: center; margin-bottom: 20px;">
           ...
           <!-- Student info -->
           <div>
               <p><strong>Name:</strong> {{ $student->first_name }} {{ $student->middle_name }} {{ $student->surname }}</p>
               ...
               <p><strong>Class Teacher:</strong> {{ $student->stream->teacher->first_name ?? 'N/A' }} {{ $student->stream->teacher->surname ?? '' }}</p>
